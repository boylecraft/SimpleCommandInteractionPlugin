...
public class ExecuteCommandInteraction extends SimpleBlockInteraction {
    public static final BuilderCodec<ExecuteCommandInteraction> CODEC;
            ...
            if (command.contains(" @a ")) {
                world.getPlayerRefs().forEach(playerRef -> {
                    CommandManager.get().handleCommand(ConsoleSender.INSTANCE,
                            (command == null || command.isEmpty()) ? "/say Hello"
                                    : command.replace("@a", playerRef.getUsername()));
                });
                ...
                world.getPlayerRefs().forEach(playerRef -> {
                    ...
                    } else if (closestPlayer.get().getTransform().getPosition().distanceTo(targetBlock) > playerRef
                            .getTransform().getPosition().distanceTo(targetBlock)) {
                        closestPlayer.set(playerRef);
                    }
                });
                if (closestPlayer.get() != null) {
                    CommandManager.get().handleCommand(ConsoleSender.INSTANCE,
                            (command == null || command.isEmpty()) ? "/say Hello"
                                    : command.replace("@p", closestPlayer.get().getUsername()));
                }
            } else if (command.contains(" @r ")) {
                Random random = new Random();
                PlayerRef playerRef = world.getPlayerRefs().stream().toList()
                        .get(random.nextInt(world.getPlayerCount()));
                if (playerRef != null) {
                    CommandManager.get().handleCommand(ConsoleSender.INSTANCE,
                            (command == null || command.isEmpty()) ? "/say Hello"
                                    : command.replace("@r", playerRef.getUsername()));
                }
            } else if (command.contains(" @s ")) {
                PlayerRef player = world.getEntityStore().getStore().getComponent(context.getEntity(),
                        PlayerRef.getComponentType());
                if (player != null) {
                    CommandManager.get().handleCommand(ConsoleSender.INSTANCE,
                            (command == null || command.isEmpty()) ? "/say Hello"
                                    : command.replace("@s", player.getUsername()));
                }
            } else {
                CommandManager.get().handleCommand(ConsoleSender.INSTANCE,
                        (command == null || command.isEmpty()) ? "/say Hello" : command);
            }
        });
    }
    ...
    static {
        CODEC = BuilderCodec
                .builder(ExecuteCommandInteraction.class, ExecuteCommandInteraction::new, SimpleBlockInteraction.CODEC)
                .documentation("Executes command when interacted with block")
                .append(new KeyedCodec<String>("Command", Codec.STRING),
